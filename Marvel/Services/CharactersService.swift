...
protocol CharactersServiceProtocol {
    ...
    func characterDetails(with id: Int, completion: @escaping (_ success: Bool, _ result: CharactersList?, _ error: String?) -> ())
}
...
class CharactersService: CharactersServiceProtocol {
    ...
    func characterDetails(with id: Int, completion: @escaping (Bool, CharactersList?, String?) -> ()) {
        let url = ApiConstants.baseUrl.appendingPathComponent("/characters/\(id)")
        let hashValue:String = ApiConstants.ts+ApiConstants.privateKey+ApiConstants.apiKey
        let digest = HashProvider().MD5(string: hashValue)
        let parameters: [String : String] = [
            "apikey": ApiConstants.apiKey,
